...
| **x-codex-skin-token** | 控制面 mutating POST 鉴权头（或 `?token=`）；`CONTROL_TOKEN_HEADER`。GET `/health` 免 token。见 SEC-01 / dual-open-policy。 |
...
| **schema 三件套** | `STATE_SCHEMA_VERSION=1`（Node 契约，非写出）· `THEME_SCHEMA_VERSION=1` · on-disk `state.json.schemaVersion=3` / `current.json.schemaVersion=1`。见 CONTRIBUTING §C-9。 |
